...
class array_var_int_element2dTest : public FznTestBase {
 public:
    ...
    if (isFixed(rowIdx)) {
      ...
      return std::none_of(colDom->begin(), colDom->end(), [&](const Int val) {
        ...
        return !isFixed(inputs.at(row).at(col)) ||
               intVal(inputs.at(row).at(col)) == outVal;
      });
    }
    if (isFixed(colIdx)) {
      const Int col = intVal(colIdx) - colOffset;
...
RC_GTEST_FIXTURE_PROP(array_var_int_element2dTest, RapidCheck, ()) {
  rapidCheck();
}
}  // namespace atlantis::testing
