...
class array_int_element2dTest : public FznTestBase {
 public:
  ...
  void generate() override {
    ...
    parameters = *rc::gen::container<std::vector<std::vector<Int>>>(
        numRows, rc::gen::container<std::vector<Int>>(
                     numCols, rc::gen::arbitrary<Int>()));

    std::vector<Int> flatPars;
        ...
        return std::none_of(
            ...
            [&](const Int rowVal) {
              return std::any_of(
                  colIdxNode.constDomain()->begin(),
                  colIdxNode.constDomain()->end(), [&](const Int colVal) {
                    return intVal(output) == getValue(rowVal, colVal);
                  });
            });
      }
      return std::none_of(
          rowIdxNode.constDomain()->begin(), rowIdxNode.constDomain()->end(),
          [&](const Int rowVal) {
            return getValue(rowVal, intVal(colIndex)) == intVal(output);
          });
    }
    if (!isFixed(colIndex)) {
      const auto& colIdxNode = _invariantGraph->varNode(colIndex);
      return std::none_of(
          colIdxNode.constDomain()->begin(), colIdxNode.constDomain()->end(),
          [&](const Int colVal) {
            return getValue(intVal(rowIndex), colVal) == intVal(output);
          });
    }
    const Int expected = getValue(intVal(rowIndex), intVal(colIndex));
    const Int actual = intVal(output);
    return expected != actual;
  }
  ...
  [[nodiscard]] bool alwaysSatisfied() const override {
    ...
    return getValue(intVal(rowIndex), intVal(colIndex)) == intVal(output);
  }
