...
class ArrayVarBoolElement2dNodeTest
 ...
 public:
  ...
  void SetUp() override {
    NodeTestBase::SetUp();
    x00 = retrieveBoolVarNode("x00");
    x01 = retrieveBoolVarNode("x01");
    x10 = retrieveBoolVarNode("x10");
    x11 = retrieveBoolVarNode("x11");
...
TEST_F(ArrayVarBoolElement2dNodeTest, replace1) {
...
TEST_F(ArrayVarBoolElement2dNodeTest, replace2) {
  EXPECT_EQ(invNode().state(), InvariantNodeState::ACTIVE);
  EXPECT_FALSE(invNode().canBeReplaced(*_invariantGraph));
