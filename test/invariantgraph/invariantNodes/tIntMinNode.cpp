...
class IntMinNodeTest : public NodeTestBase<IntMinNode> {
 public:
...
TEST_F(IntMinNodeTest, updateState) {
  EXPECT_EQ(invNode().state(), InvariantNodeState::ACTIVE);
  _invariantGraph->varNode(a).fixToValue(Int{0});
  invNode().updateState(*_invariantGraph);
  EXPECT_EQ(invNode().state(), InvariantNodeState::SUBSUMED);
  EXPECT_TRUE(_invariantGraph->varNode(output).isFixed());
}

TEST_F(IntMinNodeTest, replace) {
  EXPECT_EQ(invNode().state(), InvariantNodeState::ACTIVE);
...
TEST_F(IntMinNodeTest, propagation) {
  ...
  while (increaseNextVal(solver, inputVars, inputVals)) {
    ...
    const Int expected = *std::min_element(inputVals.begin(), inputVals.end());
