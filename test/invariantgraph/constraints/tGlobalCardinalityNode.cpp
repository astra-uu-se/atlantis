...
class AbstractGlobalCardinalityNodeTest : public NodeTestBase {
 public:
  ...
  void propagation() {
    ...
    for (auto iter = node->inputsBegin(); iter != node->inputsEnd(); ++iter) {
      EXPECT_NE((*iter)->varId(), NULL_ID);
      inputs.emplace_back((*iter)->varId());
    }
    EXPECT_EQ(inputs.size(), 2);

    std::vector<VarId> counts;
    for (auto iter = node->countsBegin(); iter != node->countsEnd(); ++iter) {
      EXPECT_NE((*iter)->varId(), NULL_ID);
      counts.emplace_back((*iter)->varId());
    }
