...
class GlobalCardinalityTest : public InvariantTest {
 public:
  ...
  void rapidCheck(size_t nVar, Int valOffset) {
    ...
    engine->makeConstraint<GlobalCardinality<IsClosed>>(viol, variables, cover,
...
class MockGlobalCardinality : public GlobalCardinality<IsClosed> {
 public:
  ...
  explicit MockGlobalCardinality(VarId violationId,
...
TEST_F(GlobalCardinalityTestClosed, EngineIntegration) {
  for (const auto& [propMode, markingMode] : propMarkModes) {
    ...
    testNotifications<MockGlobalCardinality<false>>(
        &engine->makeConstraint<MockGlobalCardinality<false>>(
...
TEST_F(GlobalCardinalityTestOpen, EngineIntegration) {
  for (const auto& [propMode, markingMode] : propMarkModes) {
    ...
    testNotifications<MockGlobalCardinality<true>>(
        &engine->makeConstraint<MockGlobalCardinality<true>>(
            viol, std::vector<VarId>{args}, std::vector<Int>{1, 2, 3},
            std::vector<Int>{1, 2, 3}),
        propMode, markingMode, numArgs + 1, args.front(), 1, viol);
  }
