...
TEST_F(BoolXorTest, UpdateBounds) {
  ...
  for (const auto& [xLb, xUb] : boundVec) {
    ...
    for (const auto& [yLb, yUb] : boundVec) {
      ...
      ASSERT_EQ(std::min(xUb, yUb), engine->upperBound(outputId));
    }
  }
}
...
class MockBoolXor : public BoolOr {
 public:
  ...
  MockBoolXor(VarId x, VarId y, VarId outputId) : BoolOr(x, y, outputId) {
...
TEST_F(BoolXorTest, EngineIntegration) {
  for (const auto& [propMode, markingMode] : propMarkModes) {
    ...
    testNotifications<MockBoolXor>(
        &engine->makeInvariant<MockBoolXor>(x, y, output), propMode,
