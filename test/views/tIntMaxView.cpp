...
RC_GTEST_FIXTURE_PROP(IntMaxViewTest, shouldAlwaysBeMax, (Int a, Int b)) {
  ...
  RC_ASSERT(view->committedValue() == std::max(a, b));
}
...
TEST_F(IntMaxViewTest, CreateIntMaxView) {
  ...
  std::shared_ptr<IntMaxView> viewOfView =
      engine->makeIntView<IntMaxView>(viewOfVar->id(), 50);

  EXPECT_EQ(viewOfVar->committedValue(), Int(25));
  EXPECT_EQ(viewOfView->committedValue(), Int(50));
...
TEST_F(IntMaxViewTest, ComputeBounds) {
  ...
  EXPECT_EQ(engine->lowerBound(va->id()), Int(-100));
  EXPECT_EQ(engine->lowerBound(vb->id()), Int(-100));
  EXPECT_EQ(engine->upperBound(va->id()), Int(10));
  EXPECT_EQ(engine->upperBound(vb->id()), Int(100));
...
TEST_F(IntMaxViewTest, RecomputeIntMaxView) {
  ...
  std::shared_ptr<IntMaxView> viewOfView =
      engine->makeIntView<IntMaxView>(viewOfVar->id(), 15);
  VarId viewOfVarId = viewOfVar->id();
  VarId viewOfViewId = viewOfView->id();

  EXPECT_EQ(engine->newValue(viewOfVarId), Int(10));
  EXPECT_EQ(engine->newValue(viewOfViewId), Int(15));

  engine->close();

  EXPECT_EQ(engine->newValue(sum), Int(40));
  EXPECT_EQ(engine->newValue(viewOfVarId), Int(40));
  EXPECT_EQ(engine->newValue(viewOfViewId), Int(40));
  ...
  EXPECT_EQ(engine->newValue(sum), Int(2));
  EXPECT_EQ(engine->newValue(viewOfVarId), Int(10));
  EXPECT_EQ(engine->newValue(viewOfViewId), Int(15));
}
...
TEST_F(IntMaxViewTest, PropagateIntViews) {
  ...
  VarId sum1ViewId = sum1View->id();
  VarId sum2ViewId = sum2View->id();
  ...
  for (size_t i = 0; i < 10; ++i) {
    ...
    sum3viewIds.emplace_back(sum3views[i]->id());
    prev = sum3viewIds[i];
  }

  EXPECT_EQ(engine->committedValue(sum1), Int(0));
  EXPECT_EQ(engine->committedValue(sum2), Int(0));
  EXPECT_EQ(engine->committedValue(sum1ViewId), Int(45));
  EXPECT_EQ(engine->committedValue(sum2ViewId), Int(20));

  engine->close();

  // a + b = 20 + 20 = sum1 = 40
  EXPECT_EQ(engine->newValue(sum1), Int(40));
  // sum1 = 40 -> sum1View = min(20, 40) = 20
  EXPECT_EQ(engine->newValue(sum1ViewId), Int(45));
  // c + d = 20 + 20 = sum2 = 40
  EXPECT_EQ(engine->newValue(sum2), Int(40));
  // sum2 = 40 -> sum2View = min(20, 40) = 20
  EXPECT_EQ(engine->newValue(sum2ViewId), Int(40));
  // sum3 = sum1view + sum2view = 45 + 40 = 85
  EXPECT_EQ(engine->newValue(sum3), Int(85));

  for (size_t i = 0; i < 10; ++i) {
    if (i == 0) {
      EXPECT_EQ(engine->committedValue(sum3viewIds[i]), Int(85));
    } else {
      EXPECT_EQ(engine->committedValue(sum3viewIds[i]),
  ...
  EXPECT_EQ(engine->newValue(a), Int(5));
  EXPECT_EQ(engine->newValue(b), Int(5));
  EXPECT_EQ(engine->newValue(c), Int(5));
  EXPECT_EQ(engine->newValue(d), Int(5));
