...
TEST_F(PropagationGraphTest, TopologicalSort) {
  ...
  // Var3, var4, var6 defines var8 via invariant 3
  ...
  // Var 1,2,4,5 are top level variables.
  // var 7 and 8 are bottom variables.

  EXPECT_NO_THROW(pg->close());
  EXPECT_EQ(pg->getTopologicalKey(VarId(1)), 0);
  EXPECT_EQ(pg->getTopologicalKey(VarId(2)), 0);
  EXPECT_EQ(pg->getTopologicalKey(InvariantId(1)), 1);
  EXPECT_EQ(pg->getTopologicalKey(VarId(3)), 2);

  EXPECT_EQ(pg->getTopologicalKey(VarId(4)), 0);
  EXPECT_EQ(pg->getTopologicalKey(VarId(5)), 0);
  EXPECT_EQ(pg->getTopologicalKey(InvariantId(2)), 1);
  EXPECT_EQ(pg->getTopologicalKey(VarId(6)), 2);
  EXPECT_EQ(pg->getTopologicalKey(VarId(7)), 2);

  EXPECT_EQ(pg->getTopologicalKey(InvariantId(3)), 3);

  EXPECT_EQ(pg->getTopologicalKey(VarId(8)), 4);
}
