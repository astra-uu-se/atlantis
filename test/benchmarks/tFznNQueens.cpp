...
TEST(FznNQueens, Solve) {
  ...
  const auto onSolution = [&foundSolution](const search::SavedAssignment& sol,
          const std::optional<
    ...
    for (size_t i = 0; i < solution.size(); ++i) {
      for (size_t j = i + 1; j < solution.size(); ++j) {
        ...
        validSolution &= (solution[i] != solution[j]) && (solution[i] + i != solution[j] + j) && solution[i] - i != solution[j] - j;
      }
      if (validSolution) {
