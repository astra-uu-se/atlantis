...
class IntMaxViewTest : public ViewTest {
 public:
  Int value{0};
  ...
  void generate() {
    _solver->open();
    makeInputVar();
    outputVar = _solver->makeIntView<IntMaxView>(*_solver, inputVar, value);
    _solver->close();
  }
...
TEST_F(IntMaxViewTest, bounds) {
  ...
  for (const auto v : values) {
    ...
    for (size_t i = 0; i < bounds.size(); ++i) {
      ...
      const Int expectedLb = std::max<Int>(value, inputLb);
      const Int expectedUb = std::max<Int>(value, inputUb);
...
RC_GTEST_FIXTURE_PROP(IntMaxViewTest, rapidcheck, ()) {
