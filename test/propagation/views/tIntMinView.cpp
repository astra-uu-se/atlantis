...
RC_GTEST_FIXTURE_PROP(IntMinViewTest, shouldAlwaysBeMin, (Int a, Int b)) {
  ...
  const VarId viewId = solver->makeIntView<IntMinView>(*solver, varId, b);
  RC_ASSERT(solver->committedValue(viewId) == std::min(a, b));
}
...
TEST_F(IntMinViewTest, CreateIntMinView) {
  ...
  const VarId viewOfVar = solver->makeIntView<IntMinView>(*solver, var, -25);
  const VarId viewOfView =
      solver->makeIntView<IntMinView>(*solver, viewOfVar, -50);
...
TEST_F(IntMinViewTest, ComputeBounds) {
  ...
  const VarId va = solver->makeIntView<IntMinView>(*solver, a, 10);
  const VarId vb = solver->makeIntView<IntMinView>(*solver, b, -200);

  EXPECT_EQ(solver->lowerBound(va), Int{-100});
  EXPECT_EQ(solver->lowerBound(vb), Int{-200});
  EXPECT_EQ(solver->upperBound(va), Int{10});
...
TEST_F(IntMinViewTest, RecomputeIntMaxView) {
  ...
  const VarId viewOfVar = solver->makeIntView<IntMinView>(*solver, sum, -10);
  const VarId viewOfView =
      solver->makeIntView<IntMinView>(*solver, viewOfVar, -15);
...
TEST_F(IntMinViewTest, PropagateIntViews) {
  ...
  const VarId sum1View = solver->makeIntView<IntMinView>(*solver, sum1, -45);
  const VarId sum2View = solver->makeIntView<IntMinView>(*solver, sum2, -20);
  ...
  for (Int i = 0; i < 10; ++i) {
    sum3views.emplace_back(
        solver->makeIntView<IntMinView>(*solver, prev, -80 - i));
    prev = sum3views[i];
  }
