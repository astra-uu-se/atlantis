...
class LessEqualConstTest : public ViewTest {
 public:
  Int value{0};
  ...
  void generate() {
    _solver->open();
    makeInputVar();
    outputVar = _solver->makeIntView<LessEqualConst>(*_solver, inputVar, value);
    _solver->close();
  }
...
TEST_F(LessEqualConstTest, bounds) {
...
RC_GTEST_FIXTURE_PROP(LessEqualConstTest, rapidcheck, ()) {
