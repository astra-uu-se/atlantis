...
TEST_F(NotEqualTest, UpdateBounds) {
  ...
  NotEqual& invariant =
      solver->makeViolationInvariant<NotEqual>(*solver, violationId, x, y);
  solver->close();
...
TEST_F(NotEqualTest, NotifyInputChanged) {
  ...
  NotEqual& invariant = solver->makeViolationInvariant<NotEqual>(
...
TEST_F(NotEqualTest, SolverIntegration) {
  for (const auto& [propMode, markingMode] : propMarkModes) {
    ...
    testNotifications<MockNotEqual>(
        &solver->makeViolationInvariant<MockNotEqual>(*solver, viol, x, y),
