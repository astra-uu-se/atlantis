...
class NotEqualTest : public InvariantTest {
 public:
  ...
  NotEqual& generate() {
    ...
    NotEqual& invariant =
        _solver->makeInvariant<NotEqual>(*_solver, outputVar, x, y);
    _solver->close();
    return invariant;
  }
};
...
class MockNotEqual : public NotEqual {
 public:
  ...
  explicit MockNotEqual(SolverBase& solver, VarViewId outputVar, VarViewId x,
                        VarViewId y)
      : NotEqual(solver, outputVar, x, y) {
