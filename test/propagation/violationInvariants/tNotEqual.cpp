...
TEST_F(NotEqualTest, UpdateBounds) {
  ...
  NotEqual& invariant =
      _solver->makeViolationInvariant<NotEqual>(*_solver, violationId, x, y);
  _solver->close();
...
TEST_F(NotEqualTest, NotifyInputChanged) {
  ...
  NotEqual& invariant = _solver->makeViolationInvariant<NotEqual>(
...
class MockNotEqual : public NotEqual {
 public:
  ...
  explicit MockNotEqual(SolverBase& _solver, VarId violationId, VarId x,
                        VarId y)
      : NotEqual(_solver, violationId, x, y) {
...
TEST_F(NotEqualTest, SolverIntegration) {
  for (const auto& [propMode, markingMode] : propMarkModes) {
    ...
    const VarId viol = _solver->makeIntVar(0, 0, 1);
    testNotifications<MockNotEqual>(
        &_solver->makeViolationInvariant<MockNotEqual>(*_solver, viol, x, y),
