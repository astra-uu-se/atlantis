...
TEST_F(EqualTest, UpdateBounds) {
  ...
  Equal& invariant =
      _solver->makeViolationInvariant<Equal>(*_solver, violationId, x, y);
  _solver->close();
...
TEST_F(EqualTest, Recompute) {
  ...
  Equal& invariant = _solver->makeViolationInvariant<Equal>(
...
class MockEqual : public Equal {
 public:
  ...
  explicit MockEqual(SolverBase& _solver, VarId violationId, VarId x, VarId y)
      : Equal(_solver, violationId, x, y) {
...
TEST_F(EqualTest, SolverIntegration) {
  for (const auto& [propMode, markingMode] : propMarkModes) {
    ...
    const VarId x = _solver->makeIntVar(5, -100, 100);
    const VarId y = _solver->makeIntVar(0, -100, 100);
