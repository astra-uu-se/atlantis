...
class BoolLessEqualTest : public InvariantTest {
 public:
  ...
  [[nodiscard]] static Int computeOutput(Int xVal, Int yVal) {
    if (xVal == 0 && yVal != 0) {
      return 1;
    }
    return 0;
  ...
  BoolLessEqual& generate() {
    ...
    BoolLessEqual& invariant =
        _solver->makeInvariant<BoolLessEqual>(*_solver, outputVar, x, y);
    _solver->close();
    return invariant;
  }
};
...
RC_GTEST_FIXTURE_PROP(BoolLessEqualTest, rapidcheck, ()) {
  _solver->open();
...
class MockBoolLessEqual : public BoolLessEqual {
 public:
  ...
  explicit MockBoolLessEqual(SolverBase& solver, VarViewId outputVar,
                             VarViewId x, VarViewId y)
      : BoolLessEqual(solver, outputVar, x, y) {
