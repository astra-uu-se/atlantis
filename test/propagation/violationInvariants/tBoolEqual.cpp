...
TEST_F(BoolEqualTest, UpdateBounds) {
  ...
  BoolEqual& invariant =
      _solver->makeViolationInvariant<BoolEqual>(*_solver, violationId, x, y);
  _solver->close();
...
TEST_F(BoolEqualTest, Recompute) {
  ...
  BoolEqual& invariant = _solver->makeViolationInvariant<BoolEqual>(
...
class MockBoolEqual : public BoolEqual {
 public:
  ...
  explicit MockBoolEqual(SolverBase& _solver, VarId violationId, VarId x,
                         VarId y)
      : BoolEqual(_solver, violationId, x, y) {
