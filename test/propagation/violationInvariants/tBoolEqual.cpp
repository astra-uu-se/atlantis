...
TEST_F(BoolEqualTest, UpdateBounds) {
  ...
  BoolEqual& invariant =
      solver->makeViolationInvariant<BoolEqual>(*solver, violationId, x, y);
  solver->close();
...
TEST_F(BoolEqualTest, Recompute) {
  ...
  BoolEqual& invariant = solver->makeViolationInvariant<BoolEqual>(
...
TEST_F(BoolEqualTest, SolverIntegration) {
  for (const auto& [propMode, markingMode] : propMarkModes) {
    ...
    testNotifications<MockBoolEqual>(
        &solver->makeViolationInvariant<MockBoolEqual>(*solver, viol, x, y),
