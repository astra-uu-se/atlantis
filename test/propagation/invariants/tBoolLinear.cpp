...
RC_GTEST_FIXTURE_PROP(BoolLinearTest, ShouldAlwaysBeSum,
  ...
  const VarId output = _solver->makeIntVar(aCoef + bCoef + cCoef,
...
class MockBoolLinear : public BoolLinear {
 public:
  ...
  explicit MockBoolLinear(SolverBase& _solver, VarId output,
                          std::vector<VarId>&& varArray)
      : BoolLinear(_solver, output, std::move(varArray)) {
