...
TEST_F(TimesTest, UpdateBounds) {
  ...
  Times& invariant = solver->makeInvariant<Times>(*solver, outputId, x, y);
  solver->close();
...
TEST_F(TimesTest, NotifyInputChanged) {
  ...
  Times& invariant = solver->makeInvariant<Times>(*solver, outputId,
...
class MockTimes : public Times {
 public:
  ...
  explicit MockTimes(SolverBase& solver, VarId output, VarId x, VarId y)
...
TEST_F(TimesTest, SolverIntegration) {
  for (const auto& [propMode, markingMode] : propMarkModes) {
    ...
    testNotifications<MockTimes>(
        &solver->makeInvariant<MockTimes>(*solver, output, x, y),
