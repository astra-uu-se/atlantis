...
TEST_F(PlusTest, UpdateBounds) {
  ...
  Plus& invariant = solver->makeInvariant<Plus>(*solver, outputId, x, y);
  solver->close();
...
TEST_F(PlusTest, NotifyInputChanged) {
  ...
  Plus& invariant = solver->makeInvariant<Plus>(*solver, outputId, inputs.at(0),
                                                inputs.at(1));
  solver->close();
...
class MockPlus : public Plus {
 public:
  ...
  explicit MockPlus(SolverBase& solver, VarId output, VarId x, VarId y)
...
TEST_F(PlusTest, SolverIntegration) {
  for (const auto& [propMode, markingMode] : propMarkModes) {
    ...
    testNotifications<MockPlus>(
        &solver->makeInvariant<MockPlus>(*solver, output, x, y),
