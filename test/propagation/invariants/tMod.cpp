...
TEST_F(ModTest, Recompute) {
  ...
  const VarId x = _solver->makeIntVar(xUb, xLb, xUb);
  const VarId y = _solver->makeIntVar(yUb, yLb, yUb);
  const VarId outputId = _solver->makeIntVar(outputLb, outputLb, outputUb);
  Mod& invariant = _solver->makeInvariant<Mod>(*_solver, outputId, x, y);
  _solver->close();
...
TEST_F(ModTest, ZeroDenominator) {
  ...
  for (const auto& [yLb, yUb, expected] : std::vector<std::array<Int, 3>>{
    ...
    for (size_t method = 0; method < 2; ++method) {
      _solver->open();
      const VarId x = _solver->makeIntVar(xVal, xVal, xVal);
      const VarId y = _solver->makeIntVar(0, yLb, yUb);
      const VarId outputId = _solver->makeIntVar(0, outputLb, outputUb);
      Mod& invariant = _solver->makeInvariant<Mod>(*_solver, outputId, x, y);
      _solver->close();
...
class MockMod : public Mod {
 public:
  ...
  explicit MockMod(SolverBase& _solver, VarId output, VarId x, VarId y)
      : Mod(_solver, output, x, y) {
