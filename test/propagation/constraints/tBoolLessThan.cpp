...
TEST_F(BoolLessThanTest, UpdateBounds) {
  ...
  BoolLessThan& invariant =
      solver->makeConstraint<BoolLessThan>(*solver, violationId, x, y);
  solver->close();
...
TEST_F(BoolLessThanTest, Recompute) {
  ...
  BoolLessThan& invariant = solver->makeConstraint<BoolLessThan>(
...
TEST_F(BoolLessThanTest, SolverIntegration) {
  for (const auto& [propMode, markingMode] : propMarkModes) {
    ...
    testNotifications<MockBoolLessThan>(
        &solver->makeConstraint<MockBoolLessThan>(*solver, viol, x, y),
