...
TEST_F(EqualTest, UpdateBounds) {
  ...
  Equal& invariant = solver->makeConstraint<Equal>(*solver, violationId, x, y);
  solver->close();
...
TEST_F(EqualTest, Recompute) {
  ...
  Equal& invariant = solver->makeConstraint<Equal>(*solver, violationId,
...
TEST_F(EqualTest, SolverIntegration) {
  for (const auto& [propMode, markingMode] : propMarkModes) {
    ...
    testNotifications<MockEqual>(
        &solver->makeConstraint<MockEqual>(*solver, viol, x, y),
