...
TEST_F(LessEqualTest, UpdateBounds) {
  ...
  LessEqual& invariant =
      solver->makeConstraint<LessEqual>(*solver, violationId, x, y);
  solver->close();
...
TEST_F(LessEqualTest, NotifyInputChanged) {
  ...
  LessEqual& invariant = solver->makeConstraint<LessEqual>(
...
TEST_F(LessEqualTest, SolverIntegration) {
  for (const auto& [propMode, markingMode] : propMarkModes) {
    ...
    testNotifications<MockLessEqual>(
        &solver->makeConstraint<MockLessEqual>(*solver, viol, x, y),
