...
HEADER="Build #$TRAVIS_BUILD_NUMBER (`git branch -v | grep \* | sed 's/\(\S*\s*\)\(([^)]*)\)*\S*\s*\([0-9a-z]*\).*/\3/'`) `git config --get remote.origin.url | sed 's/[^\.]*\.com[\:\/]*\([^\.]*\)\(\.git\)*\(@HEAD\)*/\1/'`@`git rev-parse --abbrev-ref HEAD` by `git show --quiet --pretty=format:%an`" > $BENCHMARK_FILE
