...
inline void setLogLevel(LogLevel newLogLevel) {
...
template <typename List>
void log(LogLevel logLevel, const char *file,int line,const LogData<List> &data)
{
...
#    define logDebug(x) (Logging::log(Logging::LogLevel::debug, __FILE__, __LINE__, Logging::LogData<Logging::None>() << x))
#    define logInfo(x) (Logging::log(Logging::LogLevel::info, __FILE__, __LINE__, Logging::LogData<Logging::None>() << x))
#    define logWarning(x) (Logging::log(Logging::LogLevel::warning, __FILE__, __LINE__, Logging::LogData<Logging::None>() << x))
#    define logError(x) (Logging::log(Logging::LogLevel::error, __FILE__, __LINE__, Logging::LogData<Logging::None>() << x))
#  else
#    define setLogLevel(x) do{}while(0)
#    define logDebug(x) do{}while(0)
#    define logInfo(x) do{}while(0)
#    define logWarning(x) do{}while(0)
