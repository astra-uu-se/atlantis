...
inline void PropagationQueue::initVar(const VarId id, const size_t priority) {
  assert(id == _priorityNodes.size());
  _priorityNodes.emplace_back(std::make_unique<ListNode>(id, priority));
}

inline void PropagationQueue::updatePriority(const VarId id,
                                             const size_t newPriority) {
  assert(id < _priorityNodes.size());
  _priorityNodes[id]->priority = newPriority;
}
inline void PropagationQueue::push(const VarId id) {
