...
class Committable {
 ...
 public:
  ...
  void init(Timestamp ts, T committedValue,
                                   T newValue) noexcept;
...
template <class T>
[[gnu::always_inline]] inline bool Committable<T>::hasChanged(const Timestamp ts) const {
...
template <class T>
[[gnu::always_inline]] inline T Committable<T>::get(const Timestamp currentTimestamp) const noexcept {
...
template <class T>
[[gnu::always_inline]] inline T Committable<T>::set(const Timestamp ts, T newValue) noexcept {
  _tmpTimestamp = ts;
  _tmpValue = newValue;
  return _tmpValue;
}
template <class T>
[[gnu::always_inline]] inline void Committable<T>::init(const Timestamp ts, T value) noexcept {
  init(ts, value, value);
}
template <class T>
[[gnu::always_inline]] inline void Committable<T>::init(const Timestamp ts, T committedValue,
                          T newValue) noexcept {
  _tmpTimestamp = ts;
...
template <class T>
[[gnu::always_inline]] inline void Committable<T>::commitValue(T value) noexcept {
  _committedValue = value;
}
template <class T>
[[gnu::always_inline]] inline void Committable<T>::commitIf(const Timestamp ts) noexcept {
  if (_tmpTimestamp == ts) {
