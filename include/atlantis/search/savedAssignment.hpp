 ...
 public:
  ...
  : _cost(assignment.getCost()),
    ...
    for (size_t i = 0; i < assignment.searchVars().size(); ++i) {
      _searchValues[i] = {assignment.searchVars()[i], assignment.currentValue(assignment.searchVars()[i])};
    }
  }
  ...
  [[nodiscard]] const std::vector<std::pair<propagation::VarId, Int>>& getSearchValues() const {
