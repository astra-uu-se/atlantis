...
bool array_bool_element2d(FznInvariantGraph& invariantGraph,
  ...
  invariantGraph.addInvariantNode(std::make_unique<ArrayIntElement2dNode>(
      ...
      invariantGraph.createVarNode(idx2, false), std::move(parMatrix),
      invariantGraph.createVarNode(output, true), offset1, offset2));
  return true;
}
...
bool array_bool_element2d(FznInvariantGraph& invariantGraph,
  ...
  FZN_CONSTRAINT_TYPE_CHECK(constraint, 2, fznparser::BoolVarArray, false);
  ...
  return array_bool_element2d(
      ...
      std::move(std::get<fznparser::BoolVarArray>(constraint.arguments().at(2))
                    .toParVector()),
