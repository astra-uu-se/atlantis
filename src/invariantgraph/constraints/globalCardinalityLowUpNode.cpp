...
std::unique_ptr<invariantgraph::GlobalCardinalityLowUpNode>
invariantgraph::GlobalCardinalityLowUpNode::fromModelConstraint(
  ...
  if (r != nullptr) {
    return std::make_unique<GlobalCardinalityLowUpNode>(x, cover, low, up, r);
  }
  assert(r == nullptr);
  return std::make_unique<GlobalCardinalityLowUpNode>(x, cover, low, up,
                                                      shouldHold);
}

void invariantgraph::GlobalCardinalityLowUpNode::createDefinedVariables(
    Engine& engine) {
...
void invariantgraph::GlobalCardinalityLowUpNode::registerWithEngine(
    Engine& engine) {
