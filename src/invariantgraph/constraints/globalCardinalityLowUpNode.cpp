...
GlobalCardinalityLowUpNode::fromModelConstraint(
  ...
  if (r != nullptr) {
    return std::make_unique<GlobalCardinalityLowUpNode>(inputs, cover, low, up,
                                                        r);
  }
  assert(r == nullptr);
  return std::make_unique<GlobalCardinalityLowUpNode>(inputs, cover, low, up,
...
void GlobalCardinalityLowUpNode::createDefinedVariables(Engine& engine) {
...
void GlobalCardinalityLowUpNode::registerWithEngine(Engine& engine) {
