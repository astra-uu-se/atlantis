...
std::unique_ptr<BoolXorNode> BoolXorNode::fromModelConstraint(
  ...
  if (std::holds_alternative<bool>(reified)) {
    return std::make_unique<BoolXorNode>(a, b, std::get<bool>(reified));
  }
...
void BoolXorNode::createDefinedVariables(Engine& engine) {
  registerViolation(engine);
}

void BoolXorNode::registerWithEngine(Engine& engine) {
