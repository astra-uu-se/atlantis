...
invariantgraph::GlobalCardinalityNode::GlobalCardinalityNode(
...
invariantgraph::GlobalCardinalityNode::fromModelConstraint(
  ...
  if (r != nullptr) {
    return std::make_unique<GlobalCardinalityNode>(inputs, cover, counts, r);
  }
  assert(r == nullptr);
  return std::make_unique<GlobalCardinalityNode>(inputs, cover, counts,
                                                 shouldHold);
}
