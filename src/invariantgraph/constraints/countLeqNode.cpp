...
std::unique_ptr<CountLeqNode> CountLeqNode::fromModelConstraint(
  ...
  if (yArg.isParameter()) {
    if (cArg.isParameter()) {
      if (r != nullptr) {
        return std::make_unique<CountLeqNode>(std::move(x), yParameter,
                                              cParameter, r);
      }
      return std::make_unique<CountLeqNode>(std::move(x), yParameter,
    ...
    if (r != nullptr) {
      return std::make_unique<CountLeqNode>(std::move(x), yParameter, cVarNode,
                                            r);
    }
    return std::make_unique<CountLeqNode>(std::move(x), yParameter, cVarNode,
                                          shouldHold);
  ...
  if (cArg.isParameter()) {
    if (r != nullptr) {
      return std::make_unique<CountLeqNode>(std::move(x), yVarNode, cParameter,
                                            r);
    }
    return std::make_unique<CountLeqNode>(std::move(x), yVarNode, cParameter,
                                          shouldHold);
  ...
  if (r != nullptr) {
    return std::make_unique<CountLeqNode>(std::move(x), yVarNode, cVarNode, r);
  }
  return std::make_unique<CountLeqNode>(std::move(x), yVarNode, cVarNode,
                                        shouldHold);
}

void CountLeqNode::registerWithEngine(Engine& engine) {
