...
void BoolLinLeNode::registerOutputVars(propagation::SolverBase& solver, SolverMapping& mapping) const {
  if (violationVarId(mapping) == propagation::NULL_ID) {
    ...
    if (shouldHold()) {
      ...
      setViolationVarId(solver.makeIntView<propagation::GreaterEqualConst>(
          solver, mapping.intermediateId(id()), _bound + 1), mapping);
    }
  }
...
void BoolLinLeNode::registerNode(propagation::SolverBase& solver, SolverMapping& mapping) const {
