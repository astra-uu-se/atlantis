...
std::vector<std::vector<bool>> transpose(
  ...
  std::vector<std::vector<bool>> t(table.front().size(),
...
void BoolTableIn::notifyInputChanged(const Timestamp ts, const LocalId id) {
  ...
  for (size_t r = 0; r < _transposed[id].size(); ++r) {
    ...
    const Int rowViol =
        _rowViolations[r].incValue(ts, newValue == _transposed[id][r] ? -1 : 1);
  ...
  assert(std::all_of(
      _violationCounts.begin(), _violationCounts.begin() + minViolation,
      [&](const CommittableInt& count) { return count.value(ts) == 0; }));
}
