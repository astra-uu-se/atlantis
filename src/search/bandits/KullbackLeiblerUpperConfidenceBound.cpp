...
static double findMaxQ(const size_t n, const double p, const size_t t) {
  ...
  // If n == 0 no data has yet been recorded for the arm, so we assume the
  // highest bound. p == 1 => q == 1 since q is in the range [p, 1].
  if (p == 1 || n == 0) return 1;

  // g = n * d(p,q) - upperBound
  // d(p,q) is the Kullback-Leibler divergence, in this case for a Bernoulli
  // distribution.
  ...
  const auto g = [&](const double q) {
    return p == 0 ? n * log(1 / (1 - q)) - upperBound
                  : n * (p * log(p / q) + (1 - p) * log((1 - p) / (1 - q))) -
                        upperBound;
  };
  const auto dg = [&](const double q) {
    return n * (-p / q + (1 - p) / (1 - q));
  };
...
void KullbackLeiblerUpperConfidenceBound::recordArmStats(
    const size_t arm, const PullResults& stats) {
...
KullbackLeiblerUpperConfidenceBound::chooseArm(RandomProvider& random) {
  ...
  for (size_t arm = 0; arm < _numArms; ++arm) {
    ...
    const double ucb =
        findMaxQ(_armStats[arm]->timesChosen, p, _totalRecordedPulls);
    if (ucb > largestUCB) {
      largestUCB = ucb;
      goodArms.clear();
    }
