...
void PropagationEngine::close() {
  ...
  _isOpen = false;
  std::vector<VarIdBase> decisionVariables;
  decisionVariables.reserve(_store.getNumVariables());

  for (size_t varId = 1; varId < _store.getNumVariables() + 1; ++varId) {
    if (_propGraph.getDefiningInvariant(varId) == NULL_ID &&
        !_store.getIntVar(varId).isFixed()) {
